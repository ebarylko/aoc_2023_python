...
def test_number_color_parser():
    assert number_color_parser.parse("4 green") == ["green", 4]
...
def test_line_parser():
    assert line_parser.parse("Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green\n") == [1, {"green": 2, "blue": 6,
                                                                                                 "red": 4}]
...
def test_is_valid_game():
    assert is_valid_game([12, 13, 14], {"blue": 6, "green": 2, "red": 4})
    assert not is_valid_game([12, 13, 14], {"blue": 9, "green": 11, "red": 13})
...
def test_game_power():
    assert game_power({"blue": 6, "green": 2, "red": 4}) == 48


def test_power_of_games():
    assert sum_power_of_games(example_input) == 2286
    assert sum_power_of_games(input) == 70387
